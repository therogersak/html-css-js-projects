...
const gamePlayElement = document.querySelector(".game__button-play");
const gameResetElement = document.querySelector(".game__button-reset");
...
const init = () => {
  ...
  updateData(gameHealthElement, "100%");
  ...
  if (!gameOver) {
    if (guess <= 0) {
      ...
      if (gameHealth > 20) {
        updateData(
          gameFeedbackElement,
          guess > randomGuessNumber ? "Lower!" : "Higher!"
        );
        gameHealth -= 20;
        gameHealthElement.style.width = `${gameHealth}`;
        ...
        if (gameHealth < 50) {
          ...
          gameHealth = 0;
          gameHealthElement.style.widht = `${gameHealth}`;
...
gamePlayElement.addEventListener("click", playGame);
gameResetElement.addEventListener("click", init);
